var Offers = Backbone.Collection.extend({
    ...
    fetch: async function () {
        ...
        self.url = ApiClient.offers() + '?locale=' + (locale || (DEFAULT_LANGUAGE + '_' + DEFAULT_COUNTRY));
    ...
    fetchForce: async function () {
        ...
        const locale =  await Storage.syncGet('locale');
        self.url = ApiClient.offers() + '?locale=' + locale;

        const  _success = _options.success;
        const  _error = _options.error;
    ...
    getNoTopOffers: function (id) {
        return this.filter(function (offer) {
            return offer.get("id") !== id;
        })
    }
});